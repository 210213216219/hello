...
while (
  ...
  ][--i]) && (a[i].href =  s.length > 3 ? s.padStart(10, "0") + "/" : "https://www.pedigreequery.com/" + a[i].firstChild.data + s),
i);
...
order.push(
  ctx.map((v, x) => [v[0], x + 1]).sort((a, b) => a[0] >= b[0] && -1).map(v => v[1]),
  ctx.map((v, x) => [v[1], x + 1]).sort((a, b) => a[0] <= b[0] && -1).map(v => v[1]),
  ctx.map((v, x) => [v[2], x + 1]).sort((a, b) => a[0] >= b[0] && -1).map(v => v[1]),
